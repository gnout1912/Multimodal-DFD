...
class MultimodalDataset(Dataset):
    def __init__(self, manifest_path, config, is_train=True):
        ...
        required_cols = ["face_folder", "audio_path", "label"]
        ...
        print(f"📊 {split_name} samples: {len(self.df)} | Manifest: {manifest_path}")
    ...
    def _load_video_frames(self, face_folder):
        local_folder = self._resolve_processed_path(face_folder)
        ...
        # QUAN TRỌNG:
        # Không random frame rời rạc.
        # Dùng uniform sampling để giữ thứ tự thời gian môi-tiếng.
        if len(frame_files) > self.config.MAX_FRAMES:
            ...
            # Chỉ lặp lại frame cuối nếu thiếu ít frame.
    ...
    def _load_audio(self, audio_path):
        ...
        try:
            ...
            # Nếu sample rate khác config thì bỏ qua resample ở train để tránh gọi librosa/ffmpeg.
            # Vì preprocess đã xuất wav 16kHz rồi, bình thường sr phải bằng 16000.
            if sr != self.config.AUDIO_SR:
                print(f"⚠️ Audio sample rate khác {self.config.AUDIO_SR}: {local_audio} | sr={sr}")
